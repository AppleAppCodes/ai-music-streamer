    ...
    <article className="relative flex h-full w-full snap-start snap-always items-start justify-center overflow-hidden bg-[#050505] px-4 pb-24 sm:px-8 md:pb-12">
      ...
      >
        ...
        <div className="absolute right-0 top-[calc(clamp(7rem,14vh,9rem)+min(82vw,42vh,350px)-3.4rem)] z-20 flex flex-col items-center gap-[18px] text-white" onPointerDown={(event) => event.stopPropagation()}>
          <button type="button" onClick={onFollow} className="flex flex-col items-center gap-1" aria-label={following ? t('feed.unfollow', { artist: displayArtist }) : t('feed.follow', { artist: displayArtist })}>
            <span className={`flex h-12 w-12 items-center justify-center rounded-full border-2 bg-[#10081b]/90 shadow-[0_0_24px_rgba(124,58,237,0.28)] backdrop-blur-md transition-colors ${following ? 'border-violet-300 text-white' : 'border-violet-500 text-violet-400'}`}>
              <Plus className="h-8 w-8" strokeWidth={2.3} />
            </span>
            <span className="text-[11px] font-bold text-white">{following ? t('feed.modes.following') : t('feed.followAction')}</span>
          </button>
