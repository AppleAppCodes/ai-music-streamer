...
const fmt = (value: number | null | undefined) => (value ?? 0).toLocaleString('de-DE');
...
export function registerAnalyticsTools(server: McpServer, supabase: SupabaseClient) {
    ...
    async () => {
      try {
        ...
        const activeSince = (iso: string) =>
          profiles.filter((p) => p.last_active_at && p.last_active_at >= iso).length;
        const createdSince = (iso: string) => profiles.filter((p) => p.created_at >= iso).length;

        const sources = { apple_ads: 0, organic: 0, unknown: 0 };
        const countries = new Map<string, number>();
        for (const p of profiles) {
          if (p.acquisition_source === 'apple_ads') sources.apple_ads += 1;
          else if (p.acquisition_source === 'organic') sources.organic += 1;
          else sources.unknown += 1;
  ...
  server.tool(
    ...
    async ({ days }) => {
      try {
        ...
        if (!data?.length) return { content: [{ type: 'text', text: 'Noch keine Snapshots vorhanden.' }] };
  ...
  server.tool(
    'get_top_songs_analytics',
    'Top-Songs der YORIAX-Plattform nach ECHTEN, getrackten Wiedergaben in einem Zeitfenster (nicht nach den Anzeige-Zählern). Nutze dieses Tool, wenn David wissen will, welche Songs gerade laufen oder am besten performen.',
    {
