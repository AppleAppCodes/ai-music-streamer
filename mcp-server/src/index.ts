...
server.tool(
  'upload_song',
  'Lädt einen neuen Song auf Davids Streaming-Plattform hoch. Führe dieses Tool immer aus, wenn David einen Song hochladen will. Benötigt lokale Dateipfade (absolute paths) zur Audio- und Cover-Datei auf seinem Mac.',
  {
...
server.tool(
  'list_songs',
  'List songs on the platform. Optionally filter by artist name or genre.',
  {
  ...
  async ({ artist, genre, limit, offset }) => {
    try {
      ...
      const formatted = (data || []).map((s: any) =>
        `• "${s.title}" by ${s.artist_name} [${s.genre || 'N/A'}] – ${s.plays} plays (ID: ${s.id})`
      ).join('\n');
...
server.tool(
  'delete_song',
  'Delete a song from the platform by its ID.',
  {
...
server.tool(
  'update_song_plays',
  'Update the play count (plays) of a specific song. Provide either song_id (UUID) or title (exact match).',
  {
...
server.tool(
  'list_artists',
  'List all artists on the platform with their song count and total plays.',
  {},
  async () => {
    try {
      const { data, error } = await supabase
      ...
      const artistMap = new Map<string, { songs: number; plays: number }>();
      ...
      const formatted = sorted.map(([name, stats]) =>
        `• ${name} – ${stats.songs} song(s), ${stats.plays} plays`
      ).join('\n');
...
server.tool(
  'list_playlists',
  'List all playlists, optionally filtered by user.',
  {
