...
async function resolveOrgId(config) {
    if (config.orgId)
        return config.orgId;
    if (cachedOrgId)
        return cachedOrgId;
    // /acls is called without the X-AP-Context header.
    const acls = await apiRequest(config, '/acls', { orgId: null });
...
function resolveDateRange(range, startDate, endDate) {
    ...
    const daysAgo = (n) => utcDateString(new Date(now.getTime() - n * 86_400_000));
    switch (range) {
        case 'today':
...
// ── Tool registration ───────────────────────────────────────────────────────
export function registerAppleAdsTools(server, logAction) {
    const requireConfig = () => {
    ...
    // — Orgs / setup check —
    server.tool('apple_ads_orgs', 'Apple Ads: Listet die verfügbaren Apple Search Ads Organisationen (orgId, Name, Währung). Nützlich als Setup-/Verbindungstest und um APPLE_ADS_ORG_ID zu bestimmen.', {}, async () => {
        try {
            const config = requireConfig();
            const acls = await apiRequest(config, '/acls', { orgId: null });
            const rows = (acls.data || []).map((org) => `• orgId=${org.orgId} – ${org.orgName} (${org.currency || '?'}, roles: ${Array.isArray(org.roleNames) ? org.roleNames.join('/') : '?'})`);
    ...
    // — Campaigns —
    server.tool('apple_ads_list_campaigns', 'Apple Ads: Listet alle App Store Kampagnen mit Status, Budget, Tagesbudget, Ländern/Regionen und Serving-Status.', {
        limit: z.number().int().min(1).max(1000).optional().default(50).describe('Max. Anzahl (Default 50)'),
        offset: z.number().int().min(0).optional().default(0).describe('Pagination-Offset'),
    }, async ({ limit, offset }) => {
        try {
            const config = requireConfig();
            const res = await apiRequest(config, `/campaigns?limit=${limit}&offset=${offset}`);
            const campaigns = res.data || [];
            const rows = campaigns.map((c) => {
                const countries = Array.isArray(c.countriesOrRegions) ? c.countriesOrRegions.join(', ') : '?';
                return [
                    `• [${c.id}] "${c.name}"`,
    ...
    server.tool('apple_ads_get_campaign', 'Apple Ads: Details einer einzelnen Kampagne (Status, Budgets, Länder, App-ID, Zeiten).', {
        campaign_id: z.string().describe('Kampagnen-ID'),
    }, async ({ campaign_id }) => {
        try {
            const config = requireConfig();
            const res = await apiRequest(config, `/campaigns/${campaign_id}`);
            const c = res.data;
            if (!c)
                return ok('Kampagne nicht gefunden.');
    ...
    // — Ad groups —
    server.tool('apple_ads_list_adgroups', 'Apple Ads: Listet die Ad Groups einer Kampagne (Status, Default-Bid, Serving-Status).', {
        campaign_id: z.string().describe('Kampagnen-ID'),
        ...
        try {
            const config = requireConfig();
            const res = await apiRequest(config, `/campaigns/${campaign_id}/adgroups?limit=${limit}&offset=${offset}`);
            const groups = res.data || [];
            const rows = groups.map((g) => `• [${g.id}] "${g.name}" – Status: ${g.status} (${g.displayStatus ?? g.servingStatus ?? '?'}), Default-Bid: ${fmtMoney(g.defaultBidAmount)}${g.cpaGoal ? `, CPA-Ziel: ${fmtMoney(g.cpaGoal)}` : ''}`);
    ...
    server.tool('apple_ads_list_keywords', 'Apple Ads: Listet die Targeting-Keywords einer Ad Group inkl. Match Type, Bid und Status. (Performance-Metriken pro Keyword liefert apple_ads_report mit level=keywords.)', {
        campaign_id: z.string().describe('Kampagnen-ID'),
        ...
        try {
            const config = requireConfig();
            const res = await apiRequest(config, `/campaigns/${campaign_id}/adgroups/${adgroup_id}/targetingkeywords?limit=${limit}&offset=${offset}`);
            const keywords = res.data || [];
            const rows = keywords.map((k) => `• [${k.id}] "${k.text}" – ${k.matchType}, Bid: ${fmtMoney(k.bidAmount)}, Status: ${k.status}`);
    ...
    server.tool('apple_ads_report', 'Apple Ads: Performance-Report mit Impressionen, Taps, Installs, Spend, CPT/CPA usw. level=campaigns (alle Kampagnen), adgroups/keywords/searchterms (brauchen campaign_id). Zeitraum: today, yesterday, last_7_days, last_30_days oder custom mit start_date/end_date.', {
        ...
        try {
            ...
            const { startTime, endTime } = resolveDateRange(range, start_date, end_date);
            ...
            const path = level === 'campaigns' ? '/reports/campaigns'
            ...
            const res = await apiRequest(config, path, { method: 'POST', body });
            const rows = res.data?.reportingDataResponse?.row || [];
            const grand = res.data?.reportingDataResponse?.grandTotals?.total;
            const lines = rows.map((row) => {
                const md = row.metadata || {};
                let label;
                if (md.countryOrRegion && groupBy)
                    label = `🌍 ${md.countryOrRegion} – "${md.campaignName ?? md.campaignId}"`;
                else if (md.searchTermText !== undefined)
                    label = `🔎 "${md.searchTermText ?? '(low volume)'}" [${md.searchTermSource ?? '?'}] → Keyword: "${md.keyword ?? '—'}" (${md.matchType ?? '?'})`;
                else if (md.keyword !== undefined)
                    label = `🔑 "${md.keyword}" (${md.matchType ?? '?'}, Bid: ${fmtMoney(md.bidAmount)}, Status: ${md.keywordStatus ?? '?'})`;
                else if (md.adGroupName !== undefined)
                    label = `📁 "${md.adGroupName}" [${md.adGroupId}] (${md.adGroupStatus ?? '?'})`;
                else
                    label = `📣 "${md.campaignName ?? '?'}" [${md.campaignId}] (${md.campaignStatus ?? '?'})`;
    ...
    server.tool('apple_ads_update_campaign', 'Apple Ads: Kampagne verwalten – pausieren/aktivieren und/oder Tagesbudget ändern. ACHTUNG: Wirkt sofort auf echte Werbeausgaben. Frage David IMMER zuerst um Bestätigung, bevor du dieses Tool ausführst.', {
        campaign_id: z.string().describe('Kampagnen-ID'),
        ...
        try {
            ...
            const current = await apiRequest(config, `/campaigns/${campaign_id}`);
            const currency = (current.data?.dailyBudgetAmount ?? current.data?.budgetAmount)?.currency || 'EUR';
            const campaign = {};
            if (status)
                campaign.status = status;
            if (daily_budget !== undefined)
                campaign.dailyBudgetAmount = { amount: String(daily_budget), currency };
    ...
    server.tool('apple_ads_update_keyword', 'Apple Ads: Keyword verwalten – Gebot (Bid) ändern und/oder pausieren/aktivieren. ACHTUNG: Wirkt sofort auf echte Werbeausgaben. Frage David IMMER zuerst um Bestätigung, bevor du dieses Tool ausführst.', {
        campaign_id: z.string().describe('Kampagnen-ID'),
        adgroup_id: z.string().describe('Ad-Group-ID'),
        ...
        try {
            ...
            const current = await apiRequest(config, `/campaigns/${campaign_id}`);
            const currency = (current.data?.dailyBudgetAmount ?? current.data?.budgetAmount)?.currency || 'EUR';
            const update = { id: Number(keyword_id) };
            if (bid !== undefined)
                update.bidAmount = { amount: String(bid), currency };
            if (status)
